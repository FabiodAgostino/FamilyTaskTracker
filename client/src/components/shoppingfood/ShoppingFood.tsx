// ==============================================================================
// SHOPPING FOOD COMPONENT - Versione Corretta e Rigorosamente Bilanciata
// ==============================================================================
...
export function ShoppingFoodComponent({ className }: ShoppingFoodComponentProps) {
  ...
  const [showFilters, setShowFilters] = useState(!isMobile); // Mantenuto per filtri, non per badge
  ...
  // Stats per i badge (calcolate su TUTTE le liste VISIBILI, non solo quelle filtrate per la lista)
  const allListsStats = useMemo(() => {
    const totalLists = shoppingLists.length;
    const completedLists = shoppingLists.filter(list => list.areAllItemsCompleted()).length;
    const publicLists = shoppingLists.filter(list => list.isPublic).length;
    const privateLists = shoppingLists.filter(list => !list.isPublic).length;
    const myLists = shoppingLists.filter(list => list.createdBy === user?.username).length;

    // Conteggio totale prodotti e completati su tutte le liste
    let totalItems = 0;
    let totalItemsCompleted = 0;
    let totalItemsUncategorized = 0;
    let totalItemsCategorizedAI = 0;

    shoppingLists.forEach(list => {
      totalItems += list.items?.length || 0;
      totalItemsCompleted += (list.items || []).filter(item => item.completed).length;
      totalItemsUncategorized += (list.items || []).filter(item => !item.category || item.category === 'Altro' || item.category.trim() === '').length;
      totalItemsCategorizedAI += (list.items || []).filter(item => item.assignedAutomatically).length;
    });

    const uniqueCategoriesCount = new Set<string>();
    const uniqueSupermarketsCount = new Set<string>();
    let newestListDate: Date | null = null;
    let oldestListDate: Date | null = null;
    let mostItemsList: ShoppingFood | null = null;
    let longestTitleList: ShoppingFood | null = null;
    let averageItemsPerList = 0;

    if (shoppingLists.length > 0) {
      shoppingLists.forEach(list => {
        (list.items || []).forEach(item => {
          if (item.category && item.category !== 'Altro') {
            uniqueCategoriesCount.add(item.category);
          }
        });
        if (list.supermarketId) {
          uniqueSupermarketsCount.add(list.supermarketId);
        }

        const createdAt = list.createdAt instanceof Date ? list.createdAt : new Date(list.createdAt);
        if (!newestListDate || createdAt > newestListDate) {
          newestListDate = createdAt;
        }
        if (!oldestListDate || createdAt < oldestListDate) {
          oldestListDate = createdAt;
        }

        if (!mostItemsList || (list.items?.length || 0) > (mostItemsList.items?.length || 0)) {
          mostItemsList = list;
        }
        if (!longestTitleList || list.title.length > longestTitleList.title.length) {
          longestTitleList = list;
        }
      });
      averageItemsPerList = totalItems / totalLists;
    }
    ...
    return {
      ...
      totalItems,
      totalItemsCompleted,
      totalItemsUncategorized,
      totalItemsCategorizedAI,
      uniqueCategories: uniqueCategoriesCount.size,
      uniqueSupermarkets: uniqueSupermarketsCount.size,
      newestListDate,
      oldestListDate,
      mostItemsList,
      longestTitleList,
      averageItemsPerList: parseFloat(averageItemsPerList.toFixed(1)),
    };
  }, [shoppingLists, trashedLists, user]);
    ...
    >
      <div className={cn("container mx-auto p-6 max-w-7xl", className)}>
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex-1 flex items-center gap-3"> {/* Modificato: aggiunto flex items-center gap-3 */}
            <FaCartShopping className="h-8 w-8 text-cambridge-newStyle" />
            <h1 className="text-3xl font-bold text-delft-blue">Spesa</h1>

            {/* ✅ Bottoni per popup badge e "Nuova Lista" su mobile, spostati a destra */}
            {isMobile && (
              <div className="flex items-center gap-2 ml-auto"> {/* Modificato: ml-auto per spingere a destra */}
                ...
                <DropdownMenuContent align="end">
                  ...
                  {/* Nuovo dropdown item per aprire il popup delle statistiche */}
                  <DropdownMenuItem onClick={() => setIsBadgesPopupOpen(true)}>
                    <LuChartColumnBig className="h-4 w-4 mr-2" />
          ...
          {/* Statistics Badges (visibili solo su desktop) */}
          {!isMobile && (
            <div className="flex flex-wrap gap-4">
              <Badge variant="outline" className="text-cambridge-newStyle border-cambridge-newStyle">
                {allListsStats.totalLists} {allListsStats.totalLists === 1 ? 'lista' : 'liste'}
              </Badge>
              {allListsStats.completedLists > 0 && (
                <Badge variant="outline" className="text-green-600 border-green-200">
                  {allListsStats.completedLists} completate
                </Badge>
              )}
              {allListsStats.publicLists > 0 && (
                <Badge variant="outline" className="text-blue-600 border-blue-300">
                  <Globe className="w-3 h-3 mr-1" />
                  {allListsStats.publicLists} pubbliche
                </Badge>
              )}
              {allListsStats.privateLists > 0 && (
                <Badge variant="outline" className="text-orange-600 border-orange-300">
                  <Lock className="w-3 h-3 mr-1" />
                  {allListsStats.privateLists} private
                </Badge>
              )}
              {allListsStats.myLists > 0 && (
                <Badge variant="outline" className="text-purple-600 border-purple-300">
                  <Users className="w-3 h-3 mr-1" />
                  {allListsStats.myLists} mie
                </Badge>
              )}
              {allListsStats.totalItems > 0 && (
                <Badge variant="outline" className="text-gray-600 border-gray-300">
                  <ShoppingCart className="w-3 h-3 mr-1" />
                  {allListsStats.totalItems} prodotti
                </Badge>
              )}
              {allListsStats.uniqueCategories > 0 && (
                <Badge variant="outline" className="text-cyan-600 border-cyan-300">
                  <Tag className="w-3 h-3 mr-1" />
                  {allListsStats.uniqueCategories} categorie
                </Badge>
              )}
              {allListsStats.uniqueSupermarkets > 0 && (
                <Badge variant="outline" className="text-yellow-600 border-yellow-300">
                  <Store className="w-3 h-3 mr-1" />
                  {allListsStats.uniqueSupermarkets} supermercati
                </Badge>
              )}
              {allListsStats.trashedLists > 0 && (
                <Badge variant="outline" className="text-red-600 border-red-300">
                  <Trash2 className="w-3 h-3 mr-1" />
                  {allListsStats.trashedLists} cestino
                </Badge>
              )}
            </div>
          )}
        ...
        <Card className="mb-6">
          <CardContent className="p-4">
            ...
            {!showFilters && ( /* Modificato: la sezione dei filtri si apre quando showFilters è TRUE */
                        ...
                        <div className="space-y-6 max-h-96 overflow-y-auto">
                          {(() => {
                            ...
                            for (const [categoryName, items] of grouped.entries()) {
                              ...
                              // Aggiungi anche la categoria "Altro" se esiste e non è stata già inclusa
                              if (categoryName === 'Altro' && !orderedEntries.some(entry => entry[0] === 'Altro')) {
                                orderedEntries.push([categoryName, items]);
                              }
                            }
                            ...
                            return orderedEntries.map(([categoryName, items], index) => (
                              <div key={categoryName}>
                                <div className="text-base font-semibold text-delft-blue mb-2 flex items-center gap-2">
                                  {
                                    categories?.find(c => c.name === categoryName)?.icon ?
                                      <span>{categories.find(c => c.name === categoryName)?.icon}</span> :
                                      <span>📦</span> // Icona di fallback
                                  }
                                  {categoryName}
                                ...
                                {index > 0 && <hr className="border-gray-200 my-4" />} {/* Questa riga di hr era posizionata male */}
                        ...
                        {(() => {
                          ...
                          return orderedEntries.map(([categoryName, items], index) => (
                            <div key={categoryName}>
                              ...
                              {index > 0 && <hr className="border-gray-200 my-3" />} {/* Questa riga di hr era posizionata male */}
        ...
        <Dialog open={isBadgesPopupOpen} onOpenChange={setIsBadgesPopupOpen}>
          <DialogContent className="sm:max-w-md p-6">
            <DialogHeader className="mb-4">
              <DialogTitle className="text-xl font-semibold text-delft-blue">Statistiche Dettagliate Spesa</DialogTitle>
              <DialogDescription className="text-gray-600">
                Riepilogo approfondito delle tue liste della spesa.
              </DialogDescription>
            </DialogHeader>
            <div className="flex flex-wrap gap-3 py-4 border-t border-b border-gray-200 dark:border-gray-700">
              ...
              <Badge variant="secondary" className="flex items-center justify-center p-2 text-sm font-medium bg-cambridge-newStyle/10 text-cambridge-newStyle border-cambridge-newStyle/20">
                <FaCartShopping className="w-4 h-4 mr-2" /> {allListsStats.totalLists} {allListsStats.totalLists === 1 ? 'lista' : 'liste'} totali
              </Badge>
              <Badge variant="secondary" className="flex items-center justify-center p-2 text-sm font-medium bg-green-100 text-green-700 border-green-200">
                <CheckCircle2 className="w-4 h-4 mr-2" /> {allListsStats.completedLists} {allListsStats.completedLists === 1 ? 'lista' : 'liste'} completate
              </Badge>
              <Badge variant="secondary" className="flex items-center justify-center p-2 text-sm font-medium bg-blue-100 text-blue-700 border-blue-200">
                <Globe className="w-4 h-4 mr-2" /> {allListsStats.publicLists} {allListsStats.publicLists === 1 ? 'lista' : 'liste'} pubbliche
              </Badge>
              <Badge variant="secondary" className="flex items-center justify-center p-2 text-sm font-medium bg-orange-100 text-orange-700 border-orange-200">
                <Lock className="w-4 h-4 mr-2" /> {allListsStats.privateLists} {allListsStats.privateLists === 1 ? 'lista' : 'liste'} private
              </Badge>
              <Badge variant="secondary" className="flex items-center justify-center p-2 text-sm font-medium bg-purple-100 text-purple-700 border-purple-200">
                <Users className="w-4 h-4 mr-2" /> {allListsStats.myLists} {allListsStats.myLists === 1 ? 'lista' : 'liste'} create da te
              </Badge>
              <Badge variant="secondary" className="flex items-center justify-center p-2 text-sm font-medium bg-red-100 text-red-700 border-red-200">
                <Trash2 className="w-4 h-4 mr-2" /> {allListsStats.trashedLists} {allListsStats.trashedLists === 1 ? 'lista' : 'liste'} nel cestino
              </Badge>
              {allListsStats.totalItemsCategorizedAI > 0 && (
                <Badge variant="secondary" className="flex items-center justify-center p-2 text-sm font-medium bg-indigo-100 text-indigo-700 border-indigo-200">
                  <LuChartColumnBig className="w-4 h-4 mr-2" /> {allListsStats.totalItemsCategorizedAI} prodotti categorizzati AI
                </Badge>
              )}
            </div>
