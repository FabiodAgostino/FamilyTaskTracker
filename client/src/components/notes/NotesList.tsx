// client/src/components/notes/NotesList.tsx - VERSIONE CORRETTA
import React, { useState, useMemo } from 'react';
import { Plus, Search, ChevronDown, ChevronUp, Filter } from 'lucide-react';
...
React.useEffect(() => {
  ...
  const { filteredNotes, stats } = useMemo(() => {
    ...
    const stats = {
      ...
      myNotes: user?.role === 'admin' ? visibleNotes.filter(note => note.createdBy === user.username).length : 0,
    };
  ...
  // ✅ NUOVO: Handler per chiudere il dettaglio
      ...
      <div className={cn("mb-6", isMobile ? "space-y-4" : "mb-8")}>
        <div className={cn(
          ...
          <div className={cn(isMobile ? "w-full" : "")}>
            ...
            {/* Statistics Badges - Con più spazio in mobile */}
            <div className={cn(
